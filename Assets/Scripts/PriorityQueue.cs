...
// CREDITS: https://gist.github.com/e-sarkis/716c4415254a22c2b2f9eb8d9df777f5

using UnityEngine;
#nullable enable
namespace Clrain.Collections
{
    // .Net Core c# 10 컬렉션을 불러와서 문법 다운그레이션 컨버팅
    public class PriorityQueue<TElement, TPriority>
    {
        ...
        public PriorityQueue(int initialCapacity, IComparer<TPriority>? comparer)
        {
            if (initialCapacity <= 0)
                throw new ArgumentOutOfRangeException("PriorityQueue : 초기 사이즈가 0보다 작음");
        ...
        {
            if (items == null)
                throw new ArgumentNullException("PriorityQueue : items이 null");
        ...
        {
            if (elements == null)
                throw new ArgumentNullException("PriorityQueue : elements이 Null");
            int count;
        ...
        {
            if (capacity <= 0)
                throw new ArgumentOutOfRangeException("PriorityQueue : capacity이 음수.");
        ...
        {
            UnityEngine.Debug.Assert(_nodes.Length < minCapacity);
        ...
        private void MoveUpDefaultComparer((TElement Element, TPriority Priority) node, int nodeIndex)
        {
            ...
            UnityEngine.Debug.Assert(_comparer is null);
            UnityEngine.Debug.Assert(0 <= nodeIndex && nodeIndex < _size);
        ...
        private void MoveUpCustomComparer((TElement Element, TPriority Priority) node, int nodeIndex)
        {
            ...
            UnityEngine.Debug.Assert(_comparer is not null);
            UnityEngine.Debug.Assert(0 <= nodeIndex && nodeIndex < _size);
        ...
        public sealed class UnorderedItemsCollection : IReadOnlyCollection<(TElement Element, TPriority Priority)>, ICollection
        {
            ...
            {
                if (array == null)
                    throw new ArgumentNullException("PriorityQueue : array이 null.");

                if (array.Rank != 1)
                {
                    throw new ArgumentException("PriorityQueue :배열이 1차원 배열이 아님.");
                }

                if (array.GetLowerBound(0) != 0)
                {
                    throw new ArgumentException("PriorityQueue :배열의 인덱스가 0부터 시작하지 않음.");
                }

                if (index < 0 || index > array.Length)
                {
                    throw new ArgumentOutOfRangeException("PriorityQueue :인덱스가 배열 범위 초과.");
                }

                if (array.Length - index < _queue._size)
                {
                    throw new ArgumentException("PriorityQueue : 카피할 대상의 공간이 부족함.(용량 초과)");
                }
                ...
                {
                    throw new ArgumentException("PriorityQueue : 배열의 타입이 맞지 않음.");
                }
            }
            ...
            public struct Enumerator : IEnumerator<(TElement Element, TPriority Priority)>
            {
                ...
                private bool MoveNextRare()
                {
                    ...
                    {
                        throw new InvalidOperationException("PriorityQueue : 이뉴머레이터와 원본의 타입에 미스매칭 발생. 반복자 반복중 List Size 변경.");
                    }
